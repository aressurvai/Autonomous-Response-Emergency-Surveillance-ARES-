...
const DUMMY_ALERTS = [
  { id: 1, timestamp: '2024-03-01T10:23:15', client_id: 'client1', event_type: 'Theft', confidence: 0.91, time_in_video: '0:23', video: 'footage1.mp4' },
  { id: 2, timestamp: '2024-03-01T10:45:02', client_id: 'client2', event_type: 'Fighting', confidence: 0.87, time_in_video: '0:45', video: 'footage2.mp4' },
  { id: 3, timestamp: '2024-03-01T11:02:44', client_id: 'client1', event_type: 'Vandalism', confidence: 0.76, time_in_video: '1:02', video: 'footage1.mp4' },
  { id: 4, timestamp: '2024-03-01T11:30:11', client_id: 'client3', event_type: 'Arrest', confidence: 0.93, time_in_video: '0:15', video: 'footage3.mp4' },
  { id: 5, timestamp: '2024-03-01T12:05:33', client_id: 'client2', event_type: 'Shooting', confidence: 0.82, time_in_video: '2:05', video: 'footage2.mp4' },
  { id: 6, timestamp: '2024-03-01T12:30:22', client_id: 'client1', event_type: 'Theft', confidence: 0.78, time_in_video: '1:30', video: 'footage1.mp4' },
];
...
try {
  ...
  ipcRenderer.on('client-log', (_, { clientId, msg }) => addTerminalLine(`[Client ${clientId}] ${msg}`));
...
function handleLogin() {
  ...
  if (CREDENTIALS[user] && CREDENTIALS[user] === pass) {
    err.textContent = '';
    document.getElementById('sidebar-username').textContent = user.toUpperCase();
...
async function checkApiStatus() {
  try {
    ...
    document.getElementById('api-status-text').textContent = 'API Offline (Demo Mode)';
  }
}
...
function updateAlertsSummary(alerts) {
  const counts = { Fighting: 0, Theft: 0, Vandalism: 0, Shooting: 0, Arrest: 0 };
  alerts.forEach(a => { if (counts[a.event_type] !== undefined) counts[a.event_type]++; });
...
function filterAlerts() {
  ...
  const filtered = filter === 'all' ? allAlerts : allAlerts.filter(a => a.event_type === filter);
...
function renderTrainingCharts(history) {
  ...
  const chartDefaults = {
    ...
    scales: {
      x: { grid: { color: 'rgba(15,42,63,0.8)' }, ticks: { color: '#5a8aaa', font: { family: 'Share Tech Mono', size: 10 } } },
      y: { grid: { color: 'rgba(15,42,63,0.8)' }, ticks: { color: '#5a8aaa', font: { family: 'Share Tech Mono', size: 10 } } }
    }
  };
  ...
  accuracyChart = new Chart(document.getElementById('accuracy-chart'), {
    type: 'line',
    data: { labels, datasets: [{ data: accuracies, borderColor: '#00ff9d', backgroundColor: 'rgba(0,255,157,0.1)', borderWidth: 2, fill: true, tension: 0.4, pointBackgroundColor: '#00ff9d', pointRadius: 4 }] },
  ...
  lossChart = new Chart(document.getElementById('loss-chart'), {
    type: 'line',
    data: { labels, datasets: [{ data: losses, borderColor: '#ff3860', backgroundColor: 'rgba(255,56,96,0.1)', borderWidth: 2, fill: true, tension: 0.4, pointBackgroundColor: '#ff3860', pointRadius: 4 }] },
    options: { ...chartDefaults }
  });
...
function renderTrainingTable(history) {
  ...
  tbody.innerHTML = history.map(h => {
    const improvement = prevAcc !== null ? (h.accuracy - prevAcc).toFixed(2) : '--';
    const impClass = improvement > 0 ? 'success' : improvement < 0 ? 'danger' : '';
...
function loadStackelberg() {
  const repData = DUMMY_REPUTATION;
  updateReputationCards(repData);
  renderReputationChart(repData);
}
...
function updateReputationCards(repData) {
  repData.forEach((client, i) => {
    ...
    if (rep >= 0.6 && badge) { badge.textContent = 'SELECTED'; badge.className = 'node-badge selected'; }
    else if (badge) { badge.textContent = 'LOW REP'; badge.className = 'node-badge'; }
  });
}
  ...
  reputationChart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: repData.map(r => r.reputation >= 0.8 ? 'rgba(0,255,157,0.6)' : r.reputation >= 0.5 ? 'rgba(255,189,46,0.6)' : 'rgba(255,56,96,0.6)'),
        borderColor: repData.map(r => r.reputation >= 0.8 ? '#00ff9d' : r.reputation >= 0.5 ? '#ffbd2e' : '#ff3860'),
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: 'rgba(15,42,63,0.8)' }, ticks: { color: '#5a8aaa', font: { family: 'Share Tech Mono', size: 10 } } },
        y: { min: 0, max: 1, grid: { color: 'rgba(15,42,63,0.8)' }, ticks: { color: '#5a8aaa', font: { family: 'Share Tech Mono', size: 10 } } }
      }
    }
  });
}
...
function stopServer() {
  if (ipcRenderer) { ipcRenderer.send('stop-fl-server'); setServerStatus(false); }
}
...
function showVideoOnDashboard(clientId, filePath) {
  ...
  const placeholder = document.querySelector(`#cam-${clientId} .cam-placeholder`);
  ...
  if (video) {
    ...
    document.getElementById(`cam-${clientId}-detections`).textContent = `${filename}`;
  }
}
...
async function selectClientFootage(clientId) {
  ...
  if (ipcRenderer) {
    ...
    filePath = prompt(`Enter footage path for Client ${clientId}:`);
  }
  if (filePath) {
    clientFootagePaths[clientId] = filePath;
    const filename = filePath.split(/[\\/]/).pop();
    document.getElementById(`client-path-${clientId}`).textContent = filename;
    document.getElementById(`client-path-${clientId}`).style.color = 'var(--accent)';
...
function startClient(clientId) {
  ...
  if (!videoPath) { addTerminalLine(`Select footage for Client ${clientId} first`, 'error'); return; }
  if (ipcRenderer) {
    ipcRenderer.send('start-fl-client', { clientId, videoPath });
    setClientStatus(clientId, true);
    addTerminalLine(`Client ${clientId} starting with ${videoPath.split(/[\\/]/).pop()}`, 'success');
  } else {
    addTerminalLine('Electron not available. Run from desktop app.', 'error');
  }
}
...
function stopClient(clientId) {
  if (ipcRenderer) { ipcRenderer.send('stop-fl-client', clientId); setClientStatus(clientId, false); }
}
...
function setClientStatus(clientId, online) {
  ...
  if (pill) { pill.textContent = online ? 'ONLINE' : 'OFFLINE'; pill.className = online ? 'status-pill online' : 'status-pill'; }
...
async function uploadForClient(clientId) {
  ...
  if (ipcRenderer) {
    ...
    input.accept = 'video/*';
    input.click();
    await new Promise(res => { input.onchange = () => { file = input.files[0]; res(); }; });
    if (!file) return;
  }
  ...
  try {
    ...
    if (isApiOnline) {
      ...
      const res = await fetch(`${API_BASE}/upload-video/client${clientId}`, { method: 'POST', body: formData });
      const data = await res.json();
      alerts = data.alerts || [];
    } else {
    ...
    const filename = filePath ? filePath.split(/[\\/]/).pop() : (file ? file.name : 'video');
    ...
    document.getElementById(`cam-${clientId}-detections`).textContent = `${alerts.length} detections`;
...
function renderDetectionLog(alerts, clientId) {
  ...
  if (!alerts.length) {
    log.innerHTML = '<div class="detection-empty">No anomalies detected in footage</div>';
    return;
  }
